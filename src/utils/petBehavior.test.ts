...
describe('pickBehavior', () => {
    ...
    it('边界值与越界值都归一化处理不抛错', () => {
        ...
        expect(['lookAround', 'doze', 'hop', 'wiggle', 'stretch']).toContain(
            pickBehavior(3.7).kind,
        )
    })
    ...
    it('doze 时长限定在 6~9 秒档位', () => {
        for (const r of [0.26, 0.35, 0.45, 0.49]) {
            ...
            if (b.kind === 'doze')
                expect([6000, 7000, 8000, 9000]).toContain(b.duration)
        }
    })
...
describe('nextBehaviorDelay', () => {
    ...
    it('活跃度决定等待区间：low 18~36s / high 6~14s', () => {
        expect(nextBehaviorDelay(0, 'low')).toBe(18000)
        expect(nextBehaviorDelay(0, 'high')).toBe(6000)
        expect(nextBehaviorDelay(0.5, 'high')).toBe(10000) // 6000 + round(0.5×8000)
        expect(nextBehaviorDelay(0, 'medium')).toBe(10000)
    })
})
...
describe('resolveDisplayState', () => {
    ...
    it('AI 持续状态（thinking/working）优先于打盹', () => {
        expect(resolveDisplayState('thinking', null, true, NOW)).toBe(
            'thinking',
        )
    ...
    it('AI 临时状态（success）在有效期内显示、过期后回退', () => {
        expect(resolveDisplayState('success', NOW + 500, true, NOW)).toBe(
            'success',
        )
        expect(resolveDisplayState('success', NOW - 1, true, NOW)).toBe(
            'dozing',
        )
