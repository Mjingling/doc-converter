<template>
  <div class="panel">
    <div class="panel-head">
      <h2>{{ t("pdfExtractImages.title") }}</h2>
      <p>{{ t("pdfExtractImages.subtitle") }}</p>
    </div>
    ...
    <div class="upload-zone" @click="pickFile" @dragover.prevent @drop.prevent="onDrop">
      <div v-if="!filePath" class="zone-empty">
        ...
        <p class="zone-sub">{{ t("pdfExtractImages.dropSub") }}</p>
      </div>
    ...
    <div v-if="results.length" class="results">
      <p class="result-title">{{ t("pdfExtractImages.success", { n: results.length, dir: outDir }) }}</p>
...
import { ImageOutline, DocumentTextOutline } from "@vicons/ionicons5";
...
async function pickDir() {
  const sel = await open({ directory: true, title: t("pdfExtractImages.pickDirTitle") });
...
async function run() {
  ...
  try {
    const imgs = await pdfExtractImages(filePath.value, outDir.value);
    results.value = imgs;
    history.add({ kind: "pdfExtractImages", name: fileName.value, inputs: [filePath.value], outputs: [outDir.value], ok: true });
    message.success(t("pdfExtractImages.success", { n: imgs.length, dir: outDir.value }));
  } catch (e: any) {
